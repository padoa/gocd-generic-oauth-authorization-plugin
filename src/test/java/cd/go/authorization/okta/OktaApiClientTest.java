...
public class GenericApiClientTest {
    ...
    public void setUp() throws Exception {
        ...
        when(genericConfiguration.genericEndpoint()).thenReturn("https://example.com");
        when(genericConfiguration.clientId()).thenReturn("client-id");
        when(genericConfiguration.clientSecret()).thenReturn("client-secret");

        CallbackURL.instance().updateRedirectURL("callback-url");

        genericApiClient = new GenericApiClient(genericConfiguration);
    }
    ...
    public void shouldReturnAuthorizationServerUrl() throws Exception {
        final String authorizationServerUrl = genericApiClient.authorizationServerUrl("call-back-url");
    ...
    public void shouldFetchTokenInfoUsingAuthorizationCode() throws Exception {
        ...
        when(genericConfiguration.genericEndpoint()).thenReturn(server.url("/").toString());

        final TokenInfo tokenInfo = genericApiClient.fetchAccessToken(Collections.singletonMap("code", "some-code"));
    ...
    public void shouldFetchUserProfile() throws Exception {
        ...
        server.enqueue(new MockResponse()
                .setResponseCode(200)
                .setBody(new GenericUser("foo@bar.com", "Display Name").toJSON()));

        when(genericConfiguration.genericEndpoint()).thenReturn(server.url("/").toString());

        final GenericUser genericUser = genericApiClient.userProfile(tokenInfo);

        assertThat(genericUser.getEmail(), is("foo@bar.com"));
    ...
    public void shouldErrorOutWhenAPIRequestFails() throws Exception {
        ...
        genericApiClient.userProfile(tokenInfo);
    }
}
