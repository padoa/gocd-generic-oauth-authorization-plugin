...
public class RoleConfigValidateRequestExecutor implements RequestExecutor {
    ...
    public GoPluginApiResponse execute() throws Exception {
        final ValidationResult validationResult = new MetadataValidator().validate(request.genericRoleConfiguration());

        if (!request.genericRoleConfiguration().hasConfiguration()) {
